...
if ( $archives ) {
	...
	foreach ( $archives as $archive ) {
		...
		$text = sprintf( _x('%1$s %2$d', 'month archive', 'maranatha' ), $wp_locale->get_month( $archive->month ), $archive->year );
	...
	if ( ! empty( $instance['show_dropdown'] ) ) {
		...
		echo '		<option value="">' . _x( 'Select Month', 'archives widget', 'maranatha' ) . '</option>';
		echo $items;
		echo '	</select>';
		echo '</form>';
	}
